...
void UnsharpMaskProperties::getParameters(gstPluginWrap::ParamIdSpecMap& parameters)
{
	parameters[ARG_RADIUS_CH1] = g_param_spec_double(
		"radius1", "Radius 1", "Gaussian kernel standard deviation for channel 1.",
		DBL_MIN, DBL_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_AMOUNT_CH1] = g_param_spec_double(
		"amount1", "Amount 1", "Weight of the sum of original and blurred images for channel 1.",
		0.0, 1.0, defaultAmount, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_RADIUS_CH2] = g_param_spec_double(
		"radius2", "Radius 2", "Gaussian kernel standard deviation for channel 2.",
		DBL_MIN, DBL_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_AMOUNT_CH2] = g_param_spec_double(
		"amount2", "Amount 2", "Weight of the sum of original and blurred images for channel 2.",
		0.0, 1.0, defaultAmount, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_RADIUS_CH3] = g_param_spec_double(
		"radius3", "Radius 3", "Gaussian kernel standard deviation for channel 3.",
		DBL_MIN, DBL_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_AMOUNT_CH3] = g_param_spec_double(
		"amount3", "Amount 3", "Weight of the sum of original and blurred images for channel 3.",
		0.0, 1.0, defaultAmount, (GParamFlags)G_PARAM_READWRITE);
}
