...
void MedianFilterProperties::getParameters(gstPluginWrap::ParamIdSpecMap& parameters)
{
	parameters[ARG_RADIUS_CH1] = g_param_spec_int(
		"radius1", "Kernel radius 1", "Radius of median filter kernel for channel 1",
		0, INT_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_RADIUS_CH2] = g_param_spec_int(
		"radius2", "Kernel radius 2", "Radius of median filter kernel for channel 2",
		0, INT_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);

	parameters[ARG_RADIUS_CH3] = g_param_spec_int(
		"radius3", "Kernel radius 3", "Radius of median filter kernel for channel 3",
		0, INT_MAX, defaultRadius, (GParamFlags)G_PARAM_READWRITE);
}
...
void MedianFilter::process(uint8_t* buffer)
{
	...
	if (properties->colorSpace == MedianFilterProperties::CS_RGB)
	{
		...
		ch1 = cv::Mat(properties->bufferHeight, properties->bufferWidth, CV_8UC1, buffer);
		ch2 = cv::Mat(properties->bufferHeight, properties->bufferWidth, CV_8UC1, buffer + chLen);
		ch3 = cv::Mat(properties->bufferHeight, properties->bufferWidth, CV_8UC1, buffer + chLen + chLen);
	}
